<template>
    <div>
        ...
        <div class=" py-5 px-3 pb-0 d-flex" style="justify-content: space-between;">
            <p class="" style="font-size:2rem">Définition des codes TVA</p>
            <div class="d-flex flex-row-reverse">
                <v-btn color="primary" dark @click="enableTvas">
                    Valider
                </v-btn>
            </div>
        </div>
        <p class=" py-5 px-3 pb-0">
            Selectionnez les codes TVA que vous souhaitez utiliser dans cet exercice
        </p>
...
export default {
    ...
    methods: {
        async getTvas() {
            let url = process.env.Name_api + "/tva" + "?limit=" + this.limit + "&offset=" + this.offset;
            this.rows = await this.$myService.get(url)
        },
        async initialize() {
            this.id = this.$route.params.id
            let url = process.env.Name_api + "/exercice/" + this.id+"?params=tvas";
            const res  = await this.$myService.get(url)
            this.selected = res[0].tvas
        ...
        async enableTvas() {
            ...
            const url = process.env.Name_api + `/exercice/${this.id}/enableTvas`
