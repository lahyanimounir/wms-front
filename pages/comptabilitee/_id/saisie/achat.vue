            ...
            <v-form ref="ecritureForm">
                <v-row class="mx-0 mt-2">
                    <v-col cols="2">
                        ...
                        <v-autocomplete  hide-details v-model="editedItem.journal" return-object :rules="obligationRule" label="Journal *"
                            :items="journaux" dense placeholder="Journaux" item-text="nom" item-value="id">
                    ...
                    <v-col cols="2">
                        <!-- <label for="">Date *</label> -->
                        <v-menu ref="menu1" v-model="menu1" :close-on-content-click="false" transition="scale-transition" label="Date*"
                            offset-y max-width="290px" min-width="290px">
                            <template v-slot:activator="{ on, attrs }" >
                                <v-text-field v-model="dateFormatted" persistent-hint v-bind="attrs" hint="JJ/MM/AAAA format"
                                prepend-icon="mdi-calendar" dense label="Date*"
                    ...
                    <v-col cols="2">
                        ...
                        <v-text-field :rules="obligationRule" v-model="editedItem.echeance" hide-details label="Echeance"
                            dense></v-text-field>
                    </v-col>
                    <v-col cols="3">
                    ...
                    <v-col cols="3" class="px-1 ">
                        ...
                        <v-text-field :rules="obligationRule" v-model="editedItem.libelle"  dense label="Libellé *"></v-text-field>
                    </v-col>
                ...
                <v-row class="mx-0 mt-0">
                    <v-col cols="2" class="pl-3 pr-1 ">
                        ...
                        <v-autocomplete :filter="getList" v-model="editedItem.plan_comptable" return-object :rules="editedItem.montant_ttc ? obligationRule : []"
                            :items="collectif" label="Plan comptable *" dense placeholder="Plan comptable" item-text="numero_compte"
                    ...
                    <v-col cols="2" class="pl-3 pr-1 ">
                        ...
                        <div class="d-flex">
                            <v-autocomplete v-model="editedItem.tiers" color="red" return-object
                                ...
                                :items="tiersShow" label="Tiers" dense placeholder="Tiers" item-text="denomination" item-value="id">
                                <template slot="selection" slot-scope="{ item }">
                                    {{ item.denomination.length > 7 && charsNumberTiers != -1
                                        ? item.denomination.substring(0, charsNumberTiers) + '..' : item.denomination }}
                                </template>
                            </v-autocomplete>
                            <div style="color: #5092f3;cursor: pointer;margin-left: 0.5rem;" @click="addTiers()" class="">
                            <div class="d-inline-block px-2"
                                style="border: 2px dotted; height: 33px; line-height: 32px; #5092f3;border-radius: 5px;cursor: pointer;">
                    ...
                    <v-col cols="3" class="pl-3 pr-1 ">
                        ...
                        <v-autocomplete :filter="getList" v-model="editedItem.compte" return-object :rules="obligationRule"
                            :items="contreparties" label="Compte contrepartie *" dense placeholder="compte de contrepartie"
                    ...
                    <v-col cols="1" class="px-1 ">
                        <!-- <label for="">Compte contrepartie *</label> -->
                        <v-text-field v-model="editedItem.taux_tva" type="number" label="Taux TVA *"
                            dense>
                    ...
                    <v-col cols="3" class="px-1 ">
                        ...
                        <v-autocomplete  v-model="editedItem.code_tva" return-object :items="filteredTvas" :rules="obligationRule"
                            label="Code TVA" dense placeholder="Code TVA" item-text="id" item-value="code">
                            <template slot="item" slot-scope="{ item }">
                                {{ item.code }} - {{ item.intitulee }}
                            </template>
                            <template slot="selection" slot-scope="{ item }">
                                {{ item.code }} - {{ item.intitulee.length > 7 ? item.intitulee.substring(0, charsNumberTva)
                                    + '...' : item.intitulee }}
                            </template>
                        </v-autocomplete>
                    </v-col>
                ...
                <v-row class="mx-0 mt-0" style="justify-content:end">
                    <v-col cols="2" class="px-1 ">
                        ...
                        <v-text-field  @blur="addDecimals()" v-model="editedItem.montant_ttc"
                            @keyup="positive()" type="number" label="MONTANT TTC" dense></v-text-field>
                    </v-col>
                    <v-col cols="2" class="px-1 ">
                        <!-- <label for="">MONTANT HT</label> -->
                        <v-text-field v-model="editedItem.montant_ht" type="number" label="MONTANT HT" dense></v-text-field>
                    </v-col>
                    <v-col cols="2" class="px-1 ">
                        <!-- <label for="">MONTANT TVA</label> -->
                        <v-text-field v-model="editedItem.montant_tva" type="number" label="MONTANT TVA" dense></v-text-field>
                    </v-col>
                   
                    <v-col cols="2" class="px-1">
                        <v-btn color="primary" small class="py-4 mr-1" @click="addEcriture()">{{ btnText }}</v-btn>
                        <v-btn v-if="isEdit" color="#EF9A9A" small class="py-4" @click="resetEcriture()">Annuler</v-btn>
                    </v-col>
                </v-row>
            </v-form>
        </v-card>
        ...
        <v-card elevation="0" class="mt-3 px-3 py-3" style="border:1px solid #ddd">
            ...
            <div class="">
                ...
                <v-alert class="mt-3" dense outlined v-if="someDebit != someCredit" type="error">
                    Les champs Débit total et Crédit doivent être égaux. (Montant restant pour équilibrer : {{ someDebit > someCredit ? someDebit - someCredit : someCredit - someDebit }})
                </v-alert>
            </div>
            <div class="mt-2">
